...
function main(ctx) {
  ...
  stopAssignments.forEach(stopAssignment => {
    ...
    // ctx.log.debug(scheduledStopPointId);
    // ctx.log.debug(stopPlaceId);
    ...
    //ctx.log.debug(scheduledStopPoint);
    //ctx.log.debug(stopPlace);    
    ...
    ctx.log.debug("StopPlace : " + spLat + " " + spLong);
    ctx.log.debug("ScheduledStopPlace : " + sspLat + " " + sspLong);

    const distance = Math.round(getDistanceFromLatLonInKm(spLat, spLong, sspLat, sspLong) * 1000);    
    ctx.log.debug("Distance : " + distance + " m");
